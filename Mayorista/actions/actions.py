    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if str(consulta)=="precio" or str(consulta)=="disponibilidad":
            with open('Productos.json', 'r') as contenido:
                ...
                dispatcher.utter_message(text= "Siempre buscamos brindar los mejores precios")
            ...
            if tracker.get_slot("usuario") != None:
                ...
                with open('Cuentas.json', 'r') as acc:
                    ...
                    message="Estas son tus compras con sus respectivas fechas de entrega:\n"
                    dispatcher.utter_message(text=message)
                    total = len(datos["cuentas"][i]["compras"])
                    for codigo in datos["cuentas"][i]["compras"]:
                        ...
                        for elemento in datos["envios"][j]["carrito"]:
                            ...
                            precio_unidad = elemento["precio_unidad"]
                        ...
                        message = message + f" \nPrecio total: ${total}\n"
                        dispatcher.utter_message(text=message)
                    message = f"Esas son todas tus compras. Total de compras realizadas con nosotros: {total}"
                    dispatcher.utter_message(text=message)
            else:
                        ...
                        while z < len(carrito):
                            ...
                            if j < len(datos_prod["productos"]) and carrito[z]["producto"] == datos_prod["productos"][j]["nombre"]:
                                ...
                                if datos_prod["productos"][j]["stock"] == 0:
                                    ...
                                    carrito[z]["precio_unidad"] = datos_prod["productos"][j]["preciobulto"]
                            else:
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        with open('Cuentas.json', 'r+') as acc:
            ...
            #Si se cobra envio, se suma el costo del envio al valor total de la compra
                ...
                total = datos["envios"][m]["valor_total"]
                message = f"Por favor, ingrese al siguiente link y complete el formulario con sus datos para completar la compra: https://www.urlparacompletarpago.com\n \nEl numero de pedido es {npedido}.\nSu pedido llegara el {date}\nValor total (incluye envio): {total}\nMuchas gracias por su compra." 
                dispatcher.utter_message(text=message)
